...
(function(){
  ...
  /* 
     ...
     constructor(options)
     init(options)
       _initPlayVideoSegmentsOverWebsockets()
  ...
  */
  ...
  };
  {
    ...
    // ----------------------------------------------------------------------------
    MsePlayer.prototype._initPlayVideoSegmentsOverWebsockets = function() {
      var t = this, d = t.d;
      var video = d.video;
      ...
      t.reset(function(err, data) {
        ...
        // Init the websocket client and subscribe to the specified stream id
        var ws = new WebSocket(d.src);
        ws.binaryType = 'arraybuffer';
        ws.addEventListener('message', function(event) {
          // A new frame has been received through websockets.
          var frame = {frameSequenceNumber:null, frameType:null, frameDataType:null, bufferData:null};
          var endianness, buffer, tbuffer, vbuffer, bufferData, p, v;
          ...
          buffer = event.data;
          tbuffer = new Uint8Array(buffer);
          vbuffer = new DataView(buffer);
            
          // ==== [media stream management header/data] ====
          p = 0;
          // Frame Sequence Number
          frame.frameSequenceNumber = vbuffer.getUint32(p, endianness); p += 32/8;
          // Frame Type
          frame.frameType = String.fromCharCode(vbuffer.getUint8(p)); p += 8/8;
          // Frame Data Type
          frame.frameDataType = String.fromCharCode(vbuffer.getUint8(p)); p += 8/8;
          
          // ==== [data buffer] ====
          if (buffer && buffer.byteLength && (buffer.byteLength-p)>0) {
            //bufferData = tbuffer.subarray(p, p+20); bufferData = bufferData.buffer;
            bufferData = buffer.slice(p); p += bufferData.byteLength;
            frame.bufferData = bufferData;
          }
          
          // === Now let's process the frame ===
          console.log('[wss] - Websocket frame => frameSequenceNumber:'+frame.frameSequenceNumber+'  frameType:'+frame.frameType+'  frameDataType:'+frame.frameDataType+'  bufferData(type):'+(frame.bufferData && Object.prototype.toString.call(frame.bufferData))+'  bufferData(length):'+(frame.bufferData && frame.bufferData.byteLength));
          var sb, trackId;
          switch (frame.frameType) {
            case 'D':
              sbs.some(function(_sb) {
                if ((frame.frameDataType=='V' && _sb.track.type=='video') ||
                    (frame.frameDataType=='S' && _sb.track.type=='audio')) { 
                  sb = _sb;
                  trackId = sb.track.id;
                  return true;
                }
              ...
              if (sb!=null) {
                ...
                sb.pendingSbSegmentsToAppend.push({ trackId:trackId, buffer:frame.bufferData });
              ...
              break;
            case 'C':
              if (d.mse && d.mse.readyState==='open')  {
                console.info('[MediaSourceExtension] - Ending stream');
                d.mse.endOfStream();
              }            
              break;
  ...
  var options = {
    ...
    src: 'ws://localhost:8090/' +  // Base websocket route from which streams are served
      '?type=sink&id=',
  ...
  switch (stream_id) {
    case 52:
      options.src += stream_id;
      options.tracks.push({id:1, codec:'avc3.4D401E', type:'video', kind:{schemeURI:'', value:''}, language:'und', name:'L-SMASH Video Handler'});
      options.tracks.push({id:2, codec:'mp4a.40.2',   type:'audio', kind:{schemeURI:'', value:''}, language:'und', name:'L-SMASH Audio Handler'});
      break;
    case 37:
    default:
      if (stream_id==null)  stream_id = 37;
      options.src += stream_id;
      options.tracks.push({id:1, codec:'avc3.64001F', type:'video', kind:{schemeURI:'', value:''}, language:'und', name:'L-SMASH Video Handler'});
      options.tracks.push({id:2, codec:'mp4a.40.2',   type:'audio', kind:{schemeURI:'', value:''}, language:'und', name:'L-SMASH Audio Handler'});
      break;
  }
